...
const StatItem: React.FC<{ label: string; value: number; bonus: number; icon: React.ReactNode; onAllocate?: () => void; canAllocate: boolean; }> = React.memo(({ label, value, bonus, icon, onAllocate, canAllocate }) => (
...
// New component to isolate skill list rendering. 
// This prevents the list from re-rendering when character HP/MP changes, provided the 'skills' array reference hasn't changed.
...
const CharacterSheet: React.FC<CharacterSheetProps> = ({ character }) => {
    ...
    useEffect(() => {
        if (prevStatsRef.current) {
            ...
            // Check for changes
            // Fix: Cast keys to string to prevent symbol index errors.
    ...
    // Memoize stat icons to prevent recreation on every render
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div className="space-y-1 text-sm">
                    <DerivedStatRow label="HP" value={derivedStats.HP.toLocaleString()} change={statChanges['HP']} labelColor="text-red-400" />
                    <DerivedStatRow label="MP" value={derivedStats.MP.toLocaleString()} change={statChanges['MP']} labelColor="text-blue-400" />
                    <DerivedStatRow label="Tấn công" value={derivedStats.ATK} change={statChanges['ATK']} />
                    <DerivedStatRow label="Công phép" value={derivedStats.MATK} change={statChanges['MATK']} />
                    <DerivedStatRow label="Phòng thủ" value={derivedStats.DEF.toFixed(2)} change={statChanges['DEF']} />
                    <DerivedStatRow label="Tốc độ" value={derivedStats.Speed.toFixed(1)} change={statChanges['Speed']} />
                    <DerivedStatRow label="Xuyên giáp" value={`${derivedStats.PENETRATION.toFixed(2)}%`} change={statChanges['PENETRATION']} />
                    <DerivedStatRow label="Né tránh" value={`${derivedStats.EVASION.toFixed(2)}%`} change={statChanges['EVASION']} />
                    <DerivedStatRow label="Chí mạng" value={`${derivedStats.CRIT_RATE.toFixed(2)}%`} change={statChanges['CRIT_RATE']} />
                    <DerivedStatRow label="Chính xác" value={`${derivedStats.ACCURACY.toFixed(2)}%`} change={statChanges['ACCURACY']} />
                    <DerivedStatRow label="Hút Máu" value={`${derivedStats.LIFESTEAL.toFixed(2)}%`} change={statChanges['LIFESTEAL']} />
                </div>
            </div>
        </div>
            ...
            <div>
                ...
                <div className="space-y-1 text-sm bg-gray-800/50 p-4 rounded-lg columns-1 sm:columns-2 gap-4">
                    {Object.entries(bonusEquipmentStats).map(([stat, value]) => {
                        if (value === undefined || value === 0 || stat in equipmentBaseStatBonuses) return null;
                        const isPercent = stat === Stat.CRIT_RATE || stat === Stat.LIFESTEAL || stat === Stat.EVASION || stat === Stat.PENETRATION || stat === Stat.ACCURACY;
                        return <DerivedStatRow key={stat} label={stat} value={isPercent ? `${Number(value).toFixed(2)}%` : Number(value).toFixed(0)} />;
                    })}
                </div>
            </div>
        )}
